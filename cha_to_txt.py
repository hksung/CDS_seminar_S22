...
filePointer = open('122.cha','r') # get the .cha file of interest
fileContent = filePointer.read() # get the contents of the file
sentIDs = {}
curID = 0

fileOutput = open('sample_122.txt','w') # we will save to this file
fileLines = fileContent.split('\n') # let's extract the lines into an array
			...
			for sent in lineSents:
				if sent != '' or " ":
					if sent not in sentIDs:
						...
						sentIDs[sent] = curID # save this word with this ID
						try: 
							fileOutput.write(sent+' ') # print the word to the output file; add a return character
						except KeyError:
							pass
fileOutput.close()